...
namespace prjChess
{
    class KnightTour : ChessAlgorithms
    {
        ...
        private bool backtracking(int x, int y, int value)
        {
            ...
            if (cnt > 0)
            {
                ...
                for (int i = 0; i < cnt && !_runSuccessful; i++)
                {
                    ...
                    if (backtracking(possibleSteps[i].x, possibleSteps[i].y, value + 1)) return true;
                    else
                    {
                        isStop = true; //Khử đệ quy
                    }
        ...
        public override bool runAlgorithm()
        {
            ...
            _runSuccessful = backtracking(startPosition.Item1, startPosition.Item2, 1);
            //printSolution();
